...
> ## ⚠️ THE QUERIES BELOW ARE STALE (2026-09-21)
>
> Every column was renamed from `snake_case` to `camelCase` in the database.
> **Postgres folds unquoted identifiers to lowercase**, so the examples here now
> fail rather than return the wrong answer — which is the good outcome, but they
> must be quoted before reuse:
>
> ```sql
> SELECT po_number FROM po_orders;    -- was
> SELECT "poNumber" FROM po_orders;   -- now (quotes are REQUIRED)
> SELECT poNumber  FROM po_orders;    -- ERROR: column "ponumber" does not exist
> ```
>
> Table names are unchanged (`po_orders`, `mainline_shipments`, …) and need no
> quoting. `models/` is the authority on column names — read the model, not this
> file. Outputs shown in the comments still reflect the data, not the spelling.
