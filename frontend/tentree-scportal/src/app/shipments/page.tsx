...
export default function Shipments() {
  ...
  useEffect(() => {
    function loadUser() {
      try {
        ...
        console.error('Failed to parse user session in Shipments page', e);
      }
    }
    loadUser();
  ...
  const fetchShipments = async () => {
    ...
    try {
      if (activeTab === 'history') {
        ...
        const [shipmentData, poData, session] = await Promise.all([
          getShipments(),
          getPurchaseOrders(),
          getSession()
        ]);
        ...
        // Merge logic to ensure all POs appear in trackers
        const merged: any[] = [];
  ...
  const handleSyncNetSuite = async () => {
    ...
    try {
      await fetch('http://127.0.0.1:5000/integrations/netsuite/pos');
  ...
  const handleUpdateGroupStatus = async (group: any[], newStatus: string) => {
    ...
    try {
      ...
      if (bookingNumber) {
        const bookingsRes = await fetch('http://127.0.0.1:5000/bookings');
        ...
        if (linked) {
          ...
          await fetch(`http://127.0.0.1:5000/bookings/${linked.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ...linked, booking_status: bookingStatus, shipment_status: newStatus }),
          });
        }
      }
  ...
  const handleUpdateStatus = async (shipment: any, newStatus: string) => {
    ...
    if (shipment.booking_number) {
      try {
        ...
        if (linkedBooking) {
          await fetch(`http://127.0.0.1:5000/bookings/${linkedBooking.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ...linkedBooking, shipment_status: newStatus, booking_status: mapShipmentStatusToBookingStatus(newStatus) })
          });
        }
          ...
          <div className="ml-auto flex gap-2">
            ...
            {activeTab === 'history' && isAdminOrLogistics && (
              <Button variant="outline" className="text-primary border-primary hover:bg-primary/10" onClick={async () => {
                ...
                try {
                  const res = await fetch('http://127.0.0.1:5000/history/sweep', { method: 'POST' });
                  const data = await res.json();
