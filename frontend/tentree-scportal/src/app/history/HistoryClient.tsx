...
export default function HistoryClient({ initialShipments }: { initialShipments: any[] }) {
  const [search, setSearch] = useState('');
  const [filterWarehouse, setFilterWarehouse] = useState('All');
  const [activeTab, setActiveTab] = useState('mainline');
  const [shipments] = useState<any[]>(initialShipments);
    ...
    <div className="p-6 space-y-4">
      ...
      <Tabs value={activeTab} onValueChange={(val) => setActiveTab(val || '')}>
        ...
        <TabsContent value="mainline">
          <HistoryTable list={mainlineList} isLoading={false} columns={sharedColumns} />
        </TabsContent>
        <TabsContent value="sms">
          <HistoryTable list={smsList} isLoading={false} columns={sharedColumns} />
        </TabsContent>
      </Tabs>
    </div>
  );
