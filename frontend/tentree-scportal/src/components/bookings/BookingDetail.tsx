...
import {
  X, CheckCircle, XCircle, FileText, Package, Truck, Building2, MapPin,
  Edit3, Trash2, Copy, Save, Layers, RefreshCw, Receipt, CheckCircle2,
  AlertCircle, AlertTriangle, Download, ArrowLeft,
} from 'lucide-react';
...
export default function BookingDetail({ booking, user, isHistory = false }: BookingDetailProps) {
  const router = useRouter();
  const backUrl = isHistory ? '/bookings/history' : '/bookings/active';
  const backLabel = isHistory ? 'History' : 'Active Bookings';
  ...
  const handleApprove = async () => {
    setIsLoading(true);
    try {
      await updateBooking(formData.id, { booking_status: 'Booking Approved', approved_at: new Date().toISOString() });
      toast.success(`Booking ${formData.booking_number} approved.`);
      router.push(backUrl);
    } catch {
  ...
  const handleDecline = async () => {
    setIsLoading(true);
    try {
      await updateBooking(formData.id, { booking_status: 'Declined', decline_reason: 'Booking details do not meet requirements.' });
      toast.success(`Booking ${formData.booking_number} declined.`);
      router.push(backUrl);
    } catch {
  ...
  const openLotDialog = async () => {
    ...
    setLotQtyInfo({ poTotal, totalReceived, totalBooked, unassigned, currentOpen, totalAvailable, po_number: mainPoNum });
    setLotFormData({ units: '', cargo_ready_date: formData.cargo_ready_date || '', mode: formData.mode || '' });
        ...
        <div className="max-w-3xl mx-auto">
            ...
            <div className="flex items-center gap-1">
              {!isEditing ? (
                <>
                  ...
                  {canDuplicate && (
                    <Button variant="ghost" size="sm" onClick={openLotDialog} className="h-8 w-8 p-0 text-muted-foreground hover:text-primary ml-1">
          ...
          <div className="flex items-start gap-4 mt-4">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-2xl font-bold truncate">{formData.booking_number}</h1>
              <div className="flex items-center gap-3 mt-1 flex-wrap">
                <span className="text-xs text-muted-foreground">Submitted {submittedLabel}</span>
                <Badge variant="secondary" className="text-xs font-bold">
                  {formData.booking_status || 'Draft'}
                </Badge>
                ...
                {formData.overbooked && (
                  <Badge variant="outline" className="text-xs font-bold bg-amber-500/10 border-amber-500/40 text-amber-700">
                    Overbooked
                  </Badge>
                )}
              </div>
                  ...
                  {formData.po_details && formData.po_details.filter((p: any) => p.po_number).length > 0 ? (
                    <div className="overflow-x-auto">
                      <table className="w-full text-left border-collapse bg-card">
                        <thead>
                          <tr className="text-[10px] font-bold text-muted-foreground border-b border-border">
                      ...
                      <div>
                        {ciData.status === 'confirmed' ? (
                          <span className="flex items-center gap-1.5 text-[11px] text-emerald-600 font-semibold">
                            ...
                            {ciData.confirmed_at ? ` · ${new Date(ciData.confirmed_at).toLocaleDateString()}` : ''}
            ...
            {dialogStep === 'choose' ? (
              ...
              <>
                ...
                <div className="p-4 space-y-4">
                  <div className="bg-muted/30 rounded-lg px-3 py-2 text-xs space-y-1 border border-border">
